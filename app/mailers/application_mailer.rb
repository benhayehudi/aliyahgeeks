class ApplicationMailer < ActionMailer::Base
  ...
  def write_posts_reminder 
    @noposts = User.left_outer_joins(:posts).where(posts: {id: nil})
    @contributed = User.joins(:posts).where('posts.created_at <= ?', 2.week.ago).uniq 
    @contributed.each do |user|
